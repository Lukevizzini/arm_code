...
`JointTrajectory` plus gripper command -> PWM bridge for the 4-DOF arm using MAVROS `CommandLong`
(`MAV_CMD_DO_SET_SERVO`).
It listens on `arm_controller/joint_trajectory` (what `joy_to_trajectory` publishes)
and commands Navigator PWM outputs (for example channels `9-16`). It can also map
`/gripper_open_close_command` to a separate Navigator output, defaulting to channel `13`.
...
- `gripper_topic` (string): gripper command topic (default `/gripper_open_close_command`).
- `gripper_channel` (int): separate PWM output for the gripper; default `13`, set `<=0` to disable.
